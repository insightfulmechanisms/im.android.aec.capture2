...
class BluetoothServer(private val context: Context) {
    ...
    fun startAdvertising() {
        startAdvertising(CaptureServices.CAPTURE_SERVICE_UUID)
    }
    ...
    fun initialize() {
        ...
        Timber.i("Server Name %s", bluetoothAdapter.name)

        peripheralManager.openGattServer()
        peripheralManager.removeAllServices()

        val cps = CaptureServices(peripheralManager)
        serviceImplementations[cps.service] = cps
